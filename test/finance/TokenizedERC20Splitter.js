...
contract('TokenSplitter', function (accounts) {
  ...
  it('multiple payees with varying shares', async function () {
    ...
    expectEvent(await this.contract.release(payee1), 'PaymentReleased', { to: payee1, amount: manifest[payee1].pending });
    expectEvent(await this.contract.release(payee2), 'PaymentReleased', { to: payee2, amount: manifest[payee2].pending });
    expectEvent(await this.contract.release(payee3), 'PaymentReleased', { to: payee3, amount: manifest[payee3].pending });
  });
});
